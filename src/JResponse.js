var { JPaginator } = require('./pagination/JPaginator')
var { JCursor } = require('./pagination/JCursor')
...
class JResponse {
  ...
  paginate (optinal, ...args) {
    if (optinal === undefined || optinal === null) return
    if (optinal instanceof JPaginator) { this.merge(optinal.get()) }
    if (optinal instanceof JCursor) {
      const maxId = args[0]
      const sinceId = args[1]
      this.merge(optinal.set(maxId, sinceId).get())
    }
    return this
  }
